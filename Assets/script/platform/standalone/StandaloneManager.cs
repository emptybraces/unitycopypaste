...
public class StandaloneManager : Utils.SingletonMB<StandaloneManager>
{
	public enum WindowMode
	{
		Windowed, Borderless, Fullscreen, Total
	}
	...
	void Start()
	{
		_windowName = $"unitycopypaste version {"1.00"}";
	...
	public void SetResolution(int w, int h, WindowMode mode)
	{
		StartCoroutine(SetResolutionCr(w, h, mode));
	}

	IEnumerator SetResolutionCr(int w, int h, WindowMode mode)
	{
		switch (mode)
		{
	...
	Vector2 GetCenteredPosition(int w, int h, WindowMode mode)
	{
		// desktop rect
		RECT desktopRect;
	...
	/// </summary>
	private bool UpdateDecorationSize()
	{
	...
	public void FileSave<T>(string path, T data) where T : class
	{
	...
	public T FileLoad<T>(string path) where T : class
	{
		if (File.Exists(path))
		{
			try
