...
namespace Utils
{
	...
	public static class Tween
	{
		...
		/// e.g.
		/// Tween.Apply(TweenEasing.OutSine, 現在経過時間, 開始値, 変化値, 合計時間);
		/// </summary>
		/// <param name="e">イージングタイプ</param>
		/// <param name="t">経過時間</param>
		/// <param name="b">始点</param>
		/// <param name="c">変化値 0.5-1.0がしたいなら、0.5, 0.5となる</param>
		/// <param name="d">最大経過時間</param>
		public static float Apply(TweenEasing e, float t, float b, float c, float d)
		{
			switch (e) {
				...
				case TweenEasing.InBounce: {
						return c - Tween.Apply(TweenEasing.OutBounce, d - t, 0f, c, d) + b;
					}
				...
				case TweenEasing.InOutBounce: {
						if (t < d / 2f) return Tween.Apply(TweenEasing.InBounce, t * 2f, 0f, c, d) * .5f + b;
						return Tween.Apply(TweenEasing.OutBounce, t * 2f - d, 0f, c, d) * .5f + c * .5f + b;
					}
